...
qwDataSite[] <- lapply(qwDataSite, as.character)
qwDataSite[is.na(qwDataSite)] <- "--"

qwDataSite <- data.frame(qwDataSite, stringsAsFactors=FALSE)

uniquePcodes <- unique(sapply(strsplit(names(qwDataSite),"\\."),function(x) x[2]))
...
for(i in uniquePcodes){
  newCol <- apply(qwDataSite[,grep(i,names(qwDataSite))], 1, paste, collapse="")
...
if(totalCols > 10){
  ...
  alignText <-  paste0(rep("X",times = totalCols-8),collapse="")
  alignText <-  paste0("l",alignText)
  align(secondTable) <- alignText
  print(secondTable,
         caption.placement="top",
         size = "\\footnotesize",
         latex.environment=NULL,
    ...
    firstTable <- xtable(wideQW,
         label="tab:QW1",
         caption="Water-quality data"
         )
  alignText <-  paste0(rep("X",times = totalCols),collapse="")
  alignText <-  paste0("l",alignText)
  align(firstTable) <- alignText
  print(firstTable,
         caption.placement="top",
         size = "\\footnotesize",
         latex.environment=NULL,
